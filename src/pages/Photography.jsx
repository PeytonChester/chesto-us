...
export default function Photography() {
  ...
  const { docs: photos, loading: photosLoading } = useCollection('photos', 'createdAt', 'desc')
  const loading = settingsLoading || photosLoading

  // Categories stay configured in admin but only show once they have photos
  const photoCategories = new Set(photos.map(p => p.category))
  const categories = allCategories.filter(c => photoCategories.has(c.slug))
        ...
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {Array.from({ length: 3 }).map((_, i) => (
              <div key={i} className="bg-chesto-charcoal/10 animate-pulse" style={{ aspectRatio: '3/2' }} />
            ))}
          </div>
        ) : categories.length === 0 ? (
